...
HTTPS_GIT := github.com/nftregen/nftregen.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=nftregen \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=nftregend \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/nftregend ./cmd/nftregend
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/nftregend.exe ./cmd/nftregend
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/nftregend
...
draw-deps:
	...
	@goviz -i ./cmd/nftregend -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c nftregen --local -f chains.yaml
endif
...
set-testnet-configs:
	nftregend config set client chain-id chainid-1
	nftregend config set client keyring-backend test
	nftregend config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | nftregend keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | nftregend keys add acc1 --recover`
