...
# then log output of file /code/bin/nftregend
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/nftregend \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/nftregend | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/nftregend /usr/bin/nftregend
